package Competitors.HellingerTree;
...
public class GHTree_Template {
    ...
    private final static Logger logger = Logger.getLogger(GHTree_Template.class.getName());
    private static GHVFDT GHvfdt;

    public GHTree_Template(int saIndex, int indexOfDenied, int indexOfGranted, int indexOfDeprived, String OPT) {
        this.OPT = OPT;
    ...
    public void deploy(Instances buffer) throws Exception {
        ...
        GHvfdt = new GHVFDT();
        GHvfdt.setModelContext(new InstancesHeader(buffer));
