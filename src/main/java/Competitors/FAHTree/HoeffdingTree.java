...
/**
 <!-- globalinfo-start --> 
 * A Hoeffding WenBinHT (VFDT) is an incremental, anytime
 * decision WenBinHT induction algorithm that is capable of learning from massive
 ...
 * otherincremental decision WenBinHT learners is that it has sound guarantees of
...
public class HoeffdingTree extends AbstractClassifier implements
  ...
  /** 
   * ADDED to get the total number of nodes of the WenBinHT
   * @return total number of nodes
   */
  public int getNumOfNodes() {
	  return m_activeLeafCount+ m_inactiveLeafCount+ m_decisionNodeCount;
  }
  ...
  public String globalInfo() {
    return "A Hoeffding WenBinHT (VFDT) is an incremental, anytime decision WenBinHT induction algorithm"
        ...
        + " of Hoeffding Trees not shared by otherincremental decision WenBinHT learners is that "
  ...
  public void updateClassifier(Instance inst) throws Exception {
    ...
    if (actualNode instanceof LearningNode) {
      actualNode.updateNode(inst);  // to check updateNode!!
  ...
  protected void trySplit(ActiveHNode node, SplitNode parent,
    ...
    if (node.numEntriesInClassDistribution() > 1) {
    	/**
         * modify possible splits according to new split criterion
         **/
    	                          /**added ActiveHNode for discrimination merit calculation*/
  ...
  public static void main(String[] args) {
    runClassifier(new weka.classifiers.trees.HoeffdingTree(), args);
  }
